...
This module on React components covers essential concepts and techniques for rendering lists and managing forms in React.

### Rendering Lists

* Introduced the map method in JavaScript for transforming data, which is useful for displaying data from external sources.
* Learned how to use the map method with JSX to render a list of components and the importance of keys for identifying elements.

### Controlled Components

* Explored controlled components that allow full control over form elements' state using React state.
* Discussed converting traditional HTML forms into React forms and the advantages of controlled components, such as improved form validation.

### Props and State

* Differentiated between props and state, emphasizing when to use each.
* Addressed the issue of props drilling and introduced the Context API as a solution for managing global state across components.
...
* React.memo from the official React docs
  ...
  * an API that can be used in conjunction with Context Providers to prevent unnecessary re-renders in top-level components in the tree. 

* useMemo from the official React docs
  * https://legacy.reactjs.org/docs/hooks-reference.html#usememo
  * a hook to guarantee referential equality on objects across rendering passes.